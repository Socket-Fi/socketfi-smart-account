use crate::account_error::AccountError;
...
fn verify_authenticator_data(
    ...
    if authenticator_data.len() < 33 {
        return Err(AccountError::InvalidAuthenticatorData);
    }
    let flags = authenticator_data.get_unchecked(32);

    if (flags & 0x01) == 0 {
        return Err(AccountError::UserPresenceRequired);
    }

    if (flags & 0x04) == 0 {
        return Err(AccountError::UserVerificationRequired);
    }

    let mut i = 0;
    while i < 32 {
        if authenticator_data.get_unchecked(i) != expected_rpid_hash.get_unchecked(i) {
            return Err(AccountError::InvalidRpIdHash);
        }
